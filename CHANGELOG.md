...
## v2.79.0（2026-09-26）· Embedding / Rerank 设定与向量层修复（串线 · 串行 · 静默失效）

**用户要求**：「修复错误，Embedding、Rerank 设定存在严重错误、串行问题。」

**① 「📦 获取模型」串到主 API（严重）**：Embedding / Rerank 区块里的该按钮走的是 `resolveApiTarget({purpose:'main'})`，
在区块已填好地址+模型的情况下**一个请求都不发**，还回报主 API 的错误（默认通道「跟随酒馆当前连接」）。
现在按 `data-ftt-api-pfx` 走**本区块自己**的地址/Key，未配置时报本区块的原因，结果按区块分开存放。

**② 补齐「选择模型」下拉**（V1 `modelSelect` 的等价物）：每个区块新增下拉（`data-ftt-model-select`），
选项来自该区块的获取结果、选中即写回该区块的模型键、当前值自动保留；获取失败时在下拉里如实显示原因。

**③ 串行 → 并行（性能）**：库向量与关键词向量是两个互相独立的请求，此前串行等待（实测两次 30ms 往返 = 63ms、
同一时刻仅 1 个在途）。现在 `Promise.all` 并行（实测 embed 段 21ms、`maxConcurrent=2`），失败语义不变。

**④ Rerank 不再白付调用**：此前先用余弦截到 TopN 再精排 → 精排只能重排这 TopN、无法改变入选集合。
现在精排前取更宽候选 `max(TopN×4, 20)`（受库大小限制、大库最多 20 条），精排后截 TopN；`stats` 增候选/保留计数。

**⑤ 换 Embedding 模型不再静默失效**：维度变化后旧缓存向量与查询向量维度不符 → `cosineSimilarity` 恒 0 →
召回**静默全空**且无任何提示。现在维度不符的条目按未命中处理并**重新嵌入**（成功覆盖缓存；失败从缓存**真删**，
避免每轮重试注定失败的比对），`stats.dimMismatch` 如实计数并写调试日志。

**⑥ 不完整 embedding 返回必须失败**（V1 同款实现缺陷）：V1/V2 都用 `new Array(n)` 建结果数组，**空槽会被
`Array.prototype.some` 跳过** → 服务端少返回几条时被判成功，调用方拿到带 `null` 的半份向量。
现在显式填 `null` 后校验，缺项一律失败并给出计数（`Embedding 返回不完整（1/3 条缺失）`，保留 V1 文案前缀）。

**⑦ 测试超时与取值口径**：区块「🧪 测试」改为按 `cfg.vectorTimeoutMs` 走（此前恒用默认 120s），
并以**屏幕上当前输入值**为准（V1 `collectApiBlock(pfx)` 同口径；读不到 DOM 时回落 cfg）。

**门禁**：新增 `tests/unit/vector-parallel.test.js`（12 断言：并行度/耗时、失败语义、候选放宽三态、
维度自愈与重嵌失败清理、不完整返回、测试超时信号）；`vector-layer.test.js` 增 A7/A7b/A8/A9/U4
（区块级获取模型、按区块分开的模型态、未配置区块的本区块报错、DOM 优先、模型下拉渲染）并把 E5 适配新文案。
详见 `docs/P10aq`。
...
- **注入条数上限**（`max*`）合计 **152 → 2980**（落在用户要求的 2000-3000）：情节 16→800、记忆 8→400、状态 30→400、场景 24→200、
  角色档案 10→200、物品 16→200、概念 10→200、计划 8→120、悬念 8→120、货币 8→100、平行事件 8→100、名册 24→80、传言 6→60；
  **条数只是候选上限** —— 真正决定注入体大小的是 `charBudget`（默认 8000 字），放宽后由预算按优先级择优填充。
- **存储上限**（`storeMax*`）合计 **3700 → 5600**：情节 400→1200、记忆 600→800、角色档案 300→400、物品 400→500、概念 600→700、
  场景 300→400、计划 200→300、悬念 200→300、名册 200→300、传言 200→300、货币 300→400；平行事件存储上限 30→200。
  保底（`storeMin*`）不变 —— 任何自动清理都不得跌破既定保底。用户仍可在 设定 → 提取记忆（召回上限）与 设定 → 遗忘（存储保底与上限）自行调整。

**② 非召回设定归位**：`currencyEnabled` / `currencyDynamicEnabled`（货币**记录口径**）与 `dimCharLimits.*`（10 项**入库硬截断**）
从「提取记忆」页搬到「分析记忆」页（新增「货币记录」「各大类单条字数上限」两节，含短提示与折叠说明）；
「提取记忆」页只留召回相关，并把原「召回参数」一节细分为 **注入预算 / 召回上限（各大类注入条数）/ 其它召回行为**。
控件总数不变（173，纯搬运）：analyze 5→17、extract 34→22。

**门禁**：新增 `tests/unit/settings-capacity.test.js`（8 断言：注入合计 2980 且只升不降 / 存储合计 5600 与保底不变 / 用户仍可改且硬截断生效 /
12 项归位且顺序正确 / 提取页只剩召回 22 项 / 分析页两节分节位置 / 控件总数 173 / 兜底值与新默认同源）；
`config-clock-golden` C1 增加「有意提高的 25 个上限键」白名单（并要求白名单内确实更高），`forget-golden` 显式固定记忆上限 600（黄金样本按其录制），
`settings-pages` P2/P2d、`vector-layer` U1、`analyze-grouping` A1-A3/A9 同步。详见 `docs/P10an`。
...
**① 问题**：该页只有「维度分组（总开关）」与「各维度独立子开关与分组」两节，末尾把 **5 个控件平铺**（分段读取楼层数 + 情节分段总结的 4 项）——没有节标题、没有说明，与上面的维度行混成一片，正是用户指出的「最底部一组」。

**② 修复（`ui/settings-pages.js#analyzePageHtml`）**：按 V1 v1.206（25505~25535）的分节口径补齐为四节 ——
① 维度分组（总开关）② 各维度独立子开关与分组（独立分组时生效）③ **分析范围**（`summaryChunkSize` + 一句短提示）
④ **情节分段总结（情节页「🧩 分段总结」）**（`plotSegmentBatchAtoms` / `protectManual` / `incremental` / `textLimit` + 短提示 + 折叠说明）。
长解释（保护时间范围、增量、字数硬截断、只归档不注入、单次最多 12 批）收进折叠块，页面上每节只留一句 ≤90 字提示。

**③ 完整性**：5 个控件键 + 代理键 `dimensionSeparate` = 6 个 `data-ftt-cfg`，各出现一次；控件表标签与顺序保持 V1 原样
（渲染层未改文案）；未登记的键回落末节「其它」（当前分析页没有该节）。

**门禁**：新增 `tests/unit/analyze-grouping.test.js`（9 断言：四节标题与顺序 / 底部两组归位与组内成员 / 控件键完整且唯一 /
两句短提示 ≤90 字与折叠说明 / 折叠块保留 V1 四段口径 / 维度分组代理键与文案回归 / 未登记键兜底 / 表内标签逐字 / 节容器完整）。详见 `docs/P10ak`。

## v2.72.0（2026-09-26）· 设定 → 投喂范围：UI 布局优化分组

**用户要求**：「新版本，设定的**投喂范围**，UI 布局做**优化分组**。」

**① 问题**：该页把 **37 个控件平铺**成一长条 —— 没有分节标题，四域修复参数还各自重复「概念修复…」「记忆修复…」前缀，找一项要滚很久；「投喂楼层」与「投喂标签白/黑名单」这两件同属「投什么进分析」的设定也被自动修复参数隔开。

**② 修复（`ui/settings-pages.js#feedPageHtml` + `style.css`）**：按用途分节并加组内子组 ——
📥 投喂楼层 →（紧跟）🏷 投喂标签自动分析 / 投喂白名单标签 / 投喂黑名单标签 → 🛠 自动修复（总开关与节奏）→
🎯 候选筛选（相关性阈值与抽查）→ 🧑 角色修复 → 🧩 **各域相关组修复**（概念 / 记忆 / 悬念 / 物品 四个子组，
用新的 `.ftt-group-title` 两级标题）→ 🧹 低调用清理；每节一句 ≤90 字短提示（`shortHintHtml`），长解释不再铺在页面上。

**③ 组内短标签**：四个子组的旋钮去掉重复前缀（「概念修复相关性阈值（0.1-0.95，默认 0.45）」→ 子组标题「概念修复」+
「相关性阈值（0.1-0.95，默认 0.45）」），**只改渲染文案**；`SETTINGS_CONTROLS.feed` 的 V1 原标签与键序不动。

**④ 完整性**：37 个控件键**一个不少、各出现一次**（+ 白/黑名单两个文本域 = 39 个 `data-ftt-cfg`）；未登记的键回落末节「其它」（当前为零）；标签范围的扫描 / 白名单 / 黑名单三节仍**相邻且顺序不变**（`feed-scan-golden` 的逐字比对继续通过）。

**门禁**：新增 `tests/unit/feed-grouping.test.js`（9 断言：9 节结构 / 顺序 / 四子组层次 / 控件键完整且唯一 / 组内短标签与表内 V1 标签并存 / 六句短提示 ≤90 字 / 未登记键兜底 / 样式两级层次 / 标签范围相邻关系）。详见 `docs/P10aj`。
