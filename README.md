...
> 文档版本：v2.8.0 ｜ 日期：2026-09-25 ｜ 状态：**可用版本 v2.8.0**（V1 对齐进度：B1–B3 ✅、B4-1 ✅、B5 ✅、B6 ✅、B7-1 ✅）
...
### 界面（v2.2.0 起：**V1 同构浮层面板**，样式逐字对齐）

| 入口 | 打开 |
| --- | --- |
| 扩展魔杖菜单「FTT记忆组件」 | **V1 同构浮层**（`#ftt-panel`，13 个分页：总览/情节/状态/角色/记忆/物品/货币/传言/计划悬念/场景/概念/平行/设置） |
| `/ftt-ui [分页id]` | 同上，可指定分页（如 `atoms`、`memories`、`settings`） |
| 右下角悬浮「FTT」按钮 | 扩展菜单入口不可用时出现，打开同一浮层 |
| `/ftt-panel` | 排障：报告抽屉容器候选命中情况并强制挂载抽屉卡片（`cfg.uiShowDrawer` 默认关闭） |

> 样式口径：V1 的 527 行 CSS 逐字并入 `style.css`，V2 渲染同名同层级 DOM，故 V1 视觉原样生效。
> V1 对齐进度：**B1（外壳/样式/13 分页）、B2（字段表/条目操作/多选批删/隐藏与速览/状态分组）、
> B3（分段批量摘要/单楼分析/协作式中断/清除已处理记录/进度动效）、
> **B4 首批（设定 14 组子页框架 + 105 个配置控件 + 数据管理导出/导入）** 已完成**；
、**B5（关系表 + 注入自查）、**B6（提示词模板编辑与迁移链）、**B7-1（快照链）** 已完成**；
> B4-2 与 B6–B9 待做（含 110 个 V1 面板动作清单与逐动作打勾表）：见 `docs/P8-功能对齐总表.md`、
> `docs/P8c-B2条目操作.md`、`docs/P8d-B3提取与楼层.md`、`docs/P8e-B4设定子页.md`、`docs/P8f-B5关系表与注入自查.md`、`docs/P8g-B6提示词编辑.md`、`docs/P8h-B7快照链.md`。
...
└── docs/                  # P0 探针报告 / 更新检查 / P1 内核平移 / P2 宿主与存储 / P3 注入闭环 / P3b 提取落库 / P4 提取编排 / P5 设定面板 / P5b 数据台 / P6 发布与终检 / P7 弹窗主界面 / P8 功能对齐总表 / P8b V1面板外壳 / P8c B2条目操作 / P8d B3提取与楼层 / P8e B4设定子页 / P8f B5关系表与注入自查 / P8g B6提示词编辑 / P8h B7快照链 / 排障-面板不显示
```
