...
| 层 | 职责 | 关键文件 |
| --- | --- | --- |
| `core/` | 纯逻辑：常量、工具、数据模型、算法、提示词（无 DOM / 无宿主） | `constants.js`、`util.js`、`model/`（runtime / scalars / atom / dims / hash / snapshot / money / segment / rel / rumor）+ `state.js` / `merge.js` —— 自 V1 逐字移植，**黄金样本强校验** |
...
+ 内容哈希 + 年龄与出生日期族 + 标量助手 + 状态容器与删除墓碑/隐藏保护，共 **66 项断言**
（批次 1：15、批次 2：23、批次 2b：16、批次 3：12）。
