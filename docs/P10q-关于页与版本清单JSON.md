...
> **v2.54.0 后续（详见 `docs/P10r-数据管理页重排与缓冲统计.md`）**：本文第 ⑤ 条的「本地缓冲」分节已重做 ——
> 原实现读错字段导致调试日志恒显示 0 条、且用内存事件数冒充持久简报数；现由 `ui/buffer-manage.js#bufferStats()`
> 统一现算（条数/上限/真实字节/最近时间），三类缓冲各自可清理；同时数据管理页按用途重排（导出/导入/删除/快照/缓冲）。
