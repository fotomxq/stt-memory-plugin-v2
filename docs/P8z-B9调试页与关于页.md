...
> **v2.53.0 变更（本页相关，详见 `docs/P10q-关于页与版本清单JSON.md`）**：本文档记录的「关于页」形态已按用户
> 要求改造 —— ① 版本清单**第一候选改为代码库 raw JSON**（本次并补齐了此前缺失的仓库根 `FTT-memory-changelog.json`，
> 由 `CHANGELOG.md` 生成且有门禁），扩展目录/相对路径降为兜底；② 状态行收敛为三态且不再暴露来源路径/时间戳/
> 扩展目录等实现细节；③ 关于页删除「V2 附加信息」开发块，只保留功能与版本更新；④ 「清理本地缓冲」迁到
> 设定 → 数据管理（`aboutCacheStats()` 统计）。本文档中「V2 仓库当前没有 `FTT-memory-changelog.json`」「成功态只能靠
> 桩验证」「关于页两按钮 / 它会是什么节」等表述均已被 v2.53.0 取代，保留原文仅作历史记录。
